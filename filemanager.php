...
class FileManager {
  ...
  function __construct() {
    ...
    $this->FM_ROOT_URL = ($is_https ? 'https' : 'http').'://'.$http_host.(!empty($root_url) ? '/'.$root_url : '');
  ...
  function show() {
    ...
    if (isset($_GET['del'])) {
      ...
      $this->fm_redirect($this->FM_SELF_URL.'?p='.urlencode($this->FM_PATH));
    }
    ...
    if (isset($_POST['group'], $_POST['zip'])) {
      ...
      if (!class_exists('ZipArchive')) {
        ...
        $this->fm_redirect(FM_SELF_URL.'?p='.urlencode($this->FM_PATH));
      }
      $files = $_POST['file'];
      if (!empty($files)) {
        chdir($path);
    ...
    if (isset($_GET['upload'])) {
      ...
      $out .= '<br />';
      $out .= '<p>';
      $out .= '<button class="btn"><i class="icon-apply"></i> Upload</button> &nbsp;<b><a href="?p='.urlencode($this->FM_PATH).'"><i class="icon-cancel"></i> Cancel</a></b>';
      $out .= '</p>';
      $out .= '</form>';
      $out .= '</div>';
    ...
    if (isset($_POST['copy'])) {
      ...
      } // array_map('$this->fm_enc', $copy_files);
      ...
      $out .= '<b><a href="?p='.urlencode($this->FM_PATH).'"><i class="icon-cancel"></i> Cancel</a></b>';
      $out .= '</p>';
      $out .= '</form>';
      $out .= '</div>';
      $out .= $this->fm_show_footer();
    ...
    if (isset($_GET['copy']) && !isset($_GET['finish'])) {
      ...
      $out .= '<p>';
      $out .= '<b><a href="?p='.urlencode($this->FM_PATH).'&amp;copy='.urlencode($copy).'&amp;finish=1"><i class="icon-apply"></i> Copy</a></b> &nbsp;';
      $out .= '<b><a href="?p='.urlencode($this->FM_PATH).'&amp;copy='.urlencode($copy).'&amp;finish=1&amp;move=1"><i class="icon-apply"></i> Move</a></b> &nbsp;';
      $out .= '<b><a href="?p='.urlencode($this->FM_PATH).'"><i class="icon-cancel"></i> Cancel</a></b>';
      $out .= '</p>';
      $out .= '<p><i>Select folder:</i></p>';
      $out .= '<ul class="folders break-word">';
      if ($parent !== false) {
        $out .= '<li><a href="?p='.urlencode($parent).'&amp;copy='.urlencode($copy).'"><i class="icon-arrow_up"></i> ..</a></li>';
      }
      foreach ($folders as $f) {
        $out .= '<li><a href="?p='.urlencode(trim($this->FM_PATH.'/'.$f, '/')).'&amp;copy='.urlencode($copy).'"><i class="icon-folder"></i> '.$this->fm_enc($this->fm_convert_win($f)).'</a></li>';
      }
      $out .= '</ul>';
      $out .= '</div>';
      $out .= $this->fm_show_footer();
    ...
    if (isset($_GET['view'])) {
      ...
      $out .= '</p>';
      $out .= '<p>';
      $out .= '<b><a href="?p='.urlencode($this->FM_PATH).'&amp;dl='.urlencode($file).'"><i class="icon-download"></i> Download</a></b> &nbsp;';
      $out .= '<b><a href="'.$this->fm_enc($file_url).'" target="_blank"><i class="icon-chain"></i> Open</a></b> &nbsp;';

      // ZIP actions
      if ($is_zip && $filenames !== false) {
        $zip_name = pathinfo($file_path, PATHINFO_FILENAME);
        $out .= '<b><a href="?p='.urlencode($this->FM_PATH).'&amp;unzip='.urlencode($file).'"><i class="icon-apply"></i> Unpack</a></b> &nbsp;';
        $out .= '<b><a href="?p='.urlencode($this->FM_PATH).'&amp;unzip='.urlencode($file).'&amp;tofolder=1" title="Unpack to '.$this->fm_enc($zip_name).'"><i class="icon-apply"></i>';
        $out .= 'Unpack to folder</a></b> &nbsp;';
      }

      $out .= '<b><a href="?p='.urlencode($this->FM_PATH).'"><i class="icon-goback"></i> Back</a></b>';
      $out .= '</p>';
      if ($is_zip) {
        // ZIP content
        if ($filenames !== false) {
        ...
        if ($this->use_ace) {
          $out .= $this->showEditor($content, urlencode($this->FM_PATH), $file, $ext);
        } else {
    ...
    if ($parent !== false) {
      ...
      $out .= '"><a href="?p='.urlencode($parent).'"><i class="icon-arrow_up"></i> ..</a></td></tr>';
    }
    ...
    foreach ($folders as $f) {
      ...
      $out .= '<td><div class="filename"><a href="?p='.urlencode(trim($this->FM_PATH.'/'.$f, '/')).'"><i class="'.$img.'"></i> '.$this->fm_enc($this->fm_convert_win($f)).'</a>';
      ...
      if (!$this->FM_IS_WIN) {
        $out .= '<td><a title="Change Permissions" href="?p='.urlencode($this->FM_PATH).'&amp;chmod='.urlencode($f).'">'.$perms.'</a></td>';
        $out .= '<td>';
      ...
      $out .= '<td>';
      $out .= '<a title="Delete" href="?p='.urlencode($this->FM_PATH).'&amp;del='.urlencode($f).'" onclick="return confirm(\'Delete folder?\');"><i class="icon-cross"></i> </a>';
      $out .= '<a title="Rename" href="#" onclick="rename(\''.$this->fm_enc($this->FM_PATH).'\', \''.$this->fm_enc($f).'\');return false;"><i class="icon-rename"></i> </a>';
      $out .= '<a title="Copy to..." href="?p=&amp;copy='.urlencode(trim($this->FM_PATH . '/' . $f, '/')).'"><i class="icon-copy"></i> </a>';
    ...
    foreach ($files as $f) {
      ...
      $filelink = '?p='.urlencode($this->FM_PATH).'&view='.urlencode($f);
      ...
      $out .= '<td>';
      $out .= '<a title="Delete" href="?p='.urlencode($this->FM_PATH).'&amp;del='.urlencode($f).'" onclick="return confirm(\'Delete file?\');"><i class="icon-cross"></i> </a>';
      $out .= '<a title="Rename" href="#" onclick="rename(\''.$this->fm_enc($this->FM_PATH).'\', \''.$this->fm_enc($f).'\');return false;"><i class="icon-rename"></i> </a>';
      $out .= '<a title="Copy to..." href="?p='.urlencode($this->FM_PATH).'&amp;copy='.urlencode(trim($this->FM_PATH . '/' . $f, '/')).'"><i class="icon-copy"></i> </a>';
      $out .= '<a title="Direct link" href="'.$this->fm_enc($this->FM_ROOT_URL.($this->FM_PATH != '' ? '/'.$this->FM_PATH : '').'/'.$f).'" target="_blank"><i class="icon-chain"></i> </a>';
      $out .= '<a title="Download" href="?p='.urlencode($this->FM_PATH).'&amp;dl='.urlencode($f).'"><i class="icon-download"></i> </a>';
      $out .= '</td></tr>';
    }
  ...
  <a href="#" onclick="invert_all();return false;"><i class="icon-checkbox_invert"></i> Invert selection</a></p>';
    $out .= '<p><input type="submit" name="delete" value="Delete" class="ui-button ui-state-default" onclick="return confirm(\'Delete selected files and folders?\');">';
    $out .= '<input type="submit" name="zip" value="Pack" class="ui-button ui-state-default" onclick="return confirm(\'Create archive?\');">';
    $out .= '<input type="submit" name="copy" value="Copy" class="ui-button ui-state-default"></p>';
    $out .= '</form>';
  ...
  function fm_show_footer() {
    $out = '</div>';
    $out .= '<script>';
    $out .= 'function newfolder(p){var n=prompt(\'New folder name\',\'folder\');if(n!==null&&n!==\'\'){window.location.search=\'p=\'+encodeURIComponent(p)+\'&newfolder=\'+encodeURIComponent(n);}}';
    $out .= 'function newfile(p){var n=prompt(\'New file name\',\'file\');if(n!==null&&n!==\'\'){window.location.search=\'p=\'+encodeURIComponent(p)+\'&newfile=\'+encodeURIComponent(n);}}';
    $out .= 'function rename(p,f){var n=prompt(\'New name\',f);if(n!==null&&n!==\'\'&&n!=f){window.location.search=\'p=\'+encodeURIComponent(p)+\'&ren=\'+encodeURIComponent(f)+\'&to=\'+encodeURIComponent(n);}}';
  ...
  function fm_show_nav_path($path) {
    ...
    $out .= '<a title="Upload files" href="?p='.urlencode($this->FM_PATH).'&amp;upload"><i class="icon-upload"></i> </a>';
